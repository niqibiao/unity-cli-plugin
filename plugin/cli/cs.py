...
# path every skill calls (~/.unity-cli-plugin/current/cli/cs.py), resolves which
# store/<version> the current project wants (per-project pin -> freshly
# bootstrapped -> latest), and runs that version's real cs.py in-process via
# runpy (no extra process, no execve). Rewritten verbatim on every install-cli,
# so its contract (store layout + pin file) must stay stable. `_UCP_STORE`
# overrides the store root (tests only).
_SHIM_SOURCE = '''\
...
def main():
    target = _resolve(_project_from_argv(sys.argv[1:]))
    if target is None:
