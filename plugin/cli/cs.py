...
# execve). Resolution (see adr/0001-cli-version-dispatch.md): runtime commands run
# the project's pinned version VERBATIM (no auto-selection, no drift) and error if
# it is unpinned or the pin is missing; setup/install-cli always run the NEWEST
# installed version; status and a bare invocation prefer the pin but fall back to
# newest so they work before a project is pinned. Rewritten verbatim on every
# install-cli, so its contract (store layout + pin file) must stay stable.
# `_UCP_STORE` overrides the store root (tests only).
_SHIM_SOURCE = '''\
...
newest; status falls back to newest when unpinned. See adr/0001-cli-version-dispatch.md."""
...
# installed version. (`status` and a bare invocation also tolerate a missing pin,
# but prefer the pin when present — handled in main, not here.)
...
def _read_pin(project):
    try:
        return json.loads((_unity_root(project) / ".unity-cli" / "cli.json").read_text("utf-8")).get("version")
...
def main():
    ...
    if not entries:
        print("unity-cli: no CLI installed in the store. Run the unity-cli-setup skill first.",
              file=sys.stderr)
        sys.exit(1)
    newest = entries[max(entries, key=_semver)]
    sub = _subcommand(argv)
    if sub in _NEWEST_CMDS:
        target = newest                       # maintenance: always the newest CLI
    else:
        want = _read_pin(_project_from_argv(argv))
        target = entries.get(want) if want else None
        if target is None:
            # `status` and a bare invocation (no subcommand / --help) must work
            # before a project is pinned, so they fall back to newest; real runtime
            # commands error rather than silently run a guessed version.
            if sub is None or sub == "status":
                target = newest
            elif want:
                print(f"unity-cli: this project is pinned to CLI {want}, which is not installed in "
                      f"the store. Run the unity-cli-setup skill to install it.", file=sys.stderr)
                sys.exit(1)
            else:
                print("unity-cli: this project has no CLI version pinned yet. "
                      "Run the unity-cli-setup skill first.", file=sys.stderr)
                sys.exit(1)
