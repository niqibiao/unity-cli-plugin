...
_PENDING_FILE = _STORE_ROOT / ".pending"       # version just bootstrapped, awaiting its first setup
...
# execve). Resolution: for `setup` (the re-pinning op) a freshly bootstrapped
# version (.pending) wins so an upgrade actually takes; otherwise the per-project
# pin wins (exact -> major.minor), then .pending, then newest. Rewritten verbatim
# on every install-cli, so its contract (store layout + pin file) must stay
# stable. `_UCP_STORE` overrides the store root (tests only).
_SHIM_SOURCE = '''\
"""unity-cli-plugin dispatch shim. Auto-generated by `cs install-cli` — do not
edit. Resolves the per-project CLI version and runs it. See docs/dual-agent-support.md."""
...
def _resolve(project, prefer_pending=False):
    entries = _entries()
    if not entries:
        return None
    try:
        pending = (_STORE / ".pending").read_text("utf-8").strip()
    except OSError:
        pending = None
    # `setup` is the (re)pinning operation: when a version was just bootstrapped
    # (.pending set), run setup AS that version so it can upgrade and re-pin the
    # project, even if the project still carries an older pin from a prior setup.
    if prefer_pending and pending and pending in entries:
    ...
    if want and want in entries:
        return entries[want]
    if want:
        mm = ".".join(str(want).split(".")[:2])
        same = [e for v, e in entries.items() if ".".join(v.split(".")[:2]) == mm]
        if same:
            return same[0]
    if pending and pending in entries:
        return entries[pending]
...
def cmd_install_cli(args):
    ...
    if rc == 0:
        ...
        # project to THIS version, not whatever happens to be newest in the store.
        try:
            _STORE_ROOT.mkdir(parents=True, exist_ok=True)
            _PENDING_FILE.write_text(get_plugin_version(_CLI_DIR) + "\n", "utf-8")
        except OSError:
            pass
