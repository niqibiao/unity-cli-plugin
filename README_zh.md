...
> [!IMPORTANT]
> **安装范围 = 这个 Unity 项目，不是全局。** skill 必须装在**本项目**的 Agent 目录里，
> 不要装到用户主目录 / 全局 skills 目录。
...
**2 · 安装 Unity 包：** 启动 AI Agent，输入 **`unity-cli setup`** —— 它会把
`com.zh1zh1.csharpconsole` 写进项目的 `Packages/manifest.json`。打开 Unity 编辑器让
Package Manager 解析它，然后输入 **`unity-cli status`** 验证。
...
> **为什么装在项目里？** skill 内置的 CLI 通过从自身文件位置向上查找来定位 Unity 项目，只有
> 装在项目内才能可靠定位 —— 装到主目录 / 全局目录则处在所有项目之外，永远找不到。
