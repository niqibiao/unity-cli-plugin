...
```bash
python "./plugins/unity-cli-plugin/cli/cs.py" command --json --project "$(pwd)" <namespace> <action> ['<args-json>']
```
...
2. Check per-project catalog JSON at `./plugins/unity-cli-plugin/catalog/*.json` — these are generated by `cs catalog sync` and stored per Unity project.
3. If no catalog JSON exists, fall back to a live query:
   ```bash
   python "./plugins/unity-cli-plugin/cli/cs.py" list-commands --type custom --json --project "$(pwd)"
   ```
4. If no catalog JSON is found, suggest the user run `$unity-cli-refresh-commands` to generate one.
...
python "./plugins/unity-cli-plugin/cli/cs.py" catalog list --json --project "$(pwd)"
```
...
4. If no matching command exists in the built-in catalog, check `./plugins/unity-cli-plugin/catalog/*.json` for custom commands
5. If no catalog JSON exists, run `list-commands --type custom` as a one-time fallback and suggest `$unity-cli-refresh-commands`
