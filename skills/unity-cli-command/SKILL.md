...
2. Check per-project catalog JSON at `${CLAUDE_PLUGIN_ROOT}/catalog/*.json` — these are generated by `cs catalog sync` and stored per Unity project.
...
4. If no matching command exists in the built-in catalog, check `${CLAUDE_PLUGIN_ROOT}/catalog/*.json` for custom commands
