...
```bash
python "./plugins/unity-cli-plugin/cli/cs.py" refresh --json --project "$(pwd)" --exit-playmode --wait 120
```
...
```bash
python "./plugins/unity-cli-plugin/cli/cs.py" refresh --json --project "$(pwd)" --wait 120
```
