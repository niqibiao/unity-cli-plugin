...
```bash
python "${CODEX_PLUGIN_ROOT}/cli/cs.py" refresh --json --project "$(pwd)" --exit-playmode --wait 60
```
...
1. Check play mode:
```bash
python "${CODEX_PLUGIN_ROOT}/cli/cs.py" command --json --project "$(pwd)" editor playmode.status
```

2. If `isPlaying: true` and you need to exit first:
```bash
python "${CODEX_PLUGIN_ROOT}/cli/cs.py" command --json --project "$(pwd)" editor playmode.exit
```

3. Trigger refresh without `--exit-playmode`:
```bash
python "${CODEX_PLUGIN_ROOT}/cli/cs.py" refresh --json --project "$(pwd)" --wait 60
```
